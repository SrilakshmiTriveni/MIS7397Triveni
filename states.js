...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Clear any existing options except the first one (NULL value)
    while (stateSelect.options.length > 1) {
        stateSelect.remove(1);
    }
    
    // Array of all US states, DC, and Puerto Rico with 2-letter codes ONLY
    const states = [
    ...
    // Sort states alphabetically by name for better user experience
    states.sort((a, b) => a.name.localeCompare(b.name));

    // Populate state dropdown - ONLY 2-letter codes in value attribute
    states.forEach(state => {
        const option = document.createElement('option');
        option.value = state.code;  // Store ONLY the 2-letter code
