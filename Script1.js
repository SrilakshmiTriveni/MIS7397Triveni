...
Version: 2.6
Description: Review button solid; DOB strict 120y rule; dynamic header date; ZIP truncation; textarea quote strip; stronger titles.
*/
...
(function () {
  try {
    ...
    if (l2) l2.textContent = ""; // single-line display
  } catch (e) {}
})();
...
(function setupHTMLTagPrevention() {
  ...
  textInputs.forEach(input => {
    input.addEventListener('input', function(){ this.value = preventHTMLTags(this.value); });
  });

  // Also prevent double quotes live in specific textareas
  const notes = document.getElementById("otherNotes");
  if (notes) {
    notes.addEventListener("input", () => { notes.value = notes.value.replace(/"/g, ""); });
  }

  // Described Symptoms: strip < > and "
  if (symptoms) {
  ...
  function calculateDateRanges() {
    ...
    const minBirthDate = new Date(today.getFullYear() - 120, today.getMonth(), today.getDate()); // exactly 120y ago
  ...
  // Preferred: dynamically set min/max from "today"
  (function setDynamicDateRanges(){
    const { today, minBirthDate } = calculateDateRanges();
    const dobMin = minBirthDate;      // exactly 120y ago (<= is INVALID below)
    const dobMax = today;             // allow up to today
  ...
  const getRadio = (name) => { const el = form.querySelector(`input[name="${name}"]:checked`); return el ? el.value : ""; };
  const getChecks = (name) => Array.from(form.querySelectorAll(`input[name="${name}"]:checked`)).map(i => i.value);
  const first5Zip = (raw) => { const m = (raw||"").match(/\d{5}/); return m?m[0]:""; };
  ...
  function hideAllSsnControls(){ if(!ssnConfirm) return; ssnConfirm.hidden = ssnYes.hidden = ssnNo.hidden = ssnEdit.hidden = true; }
  ...
  function lockAndHide(){ if(!ssn) return; ssn.readOnly = true; ssn.dataset.confirmed = "true"; hideAllSsnControls(); }
  function showEditOnly(){ if(!ssn) return; ssn.readOnly = true; ssn.dataset.confirmed = "false"; ssnConfirm.hidden=true; ssnYes.hidden=true; ssnNo.hidden=true; ssnEdit.hidden=false; }
  function unlockForEdit(){ if(!ssn) return; ssn.readOnly = false; ssn.dataset.confirmed = "false"; hideAllSsnControls(); ssn.focus(); }
  function updateSSN(){
    if(!ssn) return;
    const digits = ssn.value.replace(/\D/g, "").slice(0,9);
    ssn.value = formatSSN(digits);
    if (digits.length === 9) { ssn.setCustomValidity(""); if (ssnErr) ssnErr.textContent = ""; }
    else if (digits.length === 0) { ssn.setCustomValidity(""); if (ssnErr) ssnErr.textContent=""; hideAllSsnControls(); ssn.dataset.confirmed="false"; }
    else { ssn.setCustomValidity("SSN must be 9 digits (e.g., 123-45-6789)."); if (ssnErr) ssnErr.textContent = "SSN must be 9 digits (e.g., 123-45-6789)."; hideAllSsnControls(); }
  }
  if (ssn){
    ssn.addEventListener("input", updateSSN);
    ssn.addEventListener("blur", () => {
      const digits = ssn.value.replace(/\D/g, "");
      if (digits.length === 9) { const formatted = formatSSN(digits); ssn.value = formatted; ssn.readOnly = true; showConfirmLine(formatted); }
    });
    ssnYes?.addEventListener("click", () => { const digits = ssn.value.replace(/\D/g, ""); if (digits.length === 9) lockAndHide(); });
  ...
  const validateUserID = () => {
    ...
    userid.value = value.toLowerCase(); value = userid.value;
    if (!value) { userid.setCustomValidity("User ID is required."); userErr.textContent = "User ID is required."; return false; }
    const pat = /^(?!\d)[A-Za-z0-9_-]{5,30}$/;
    if (!pat.test(value)) {
      if (value.length < 5 || value.length > 30) { userid.setCustomValidity("User ID must be 5-30 characters."); userErr.textContent = "Must be 5-30 characters."; }
      else if (/^\d/.test(value)) { userid.setCustomValidity("User ID must start with a letter."); userErr.textContent = "Must start with a letter."; }
      else if (/\s/.test(value)) { userid.setCustomValidity("User ID cannot contain spaces."); userErr.textContent = "No spaces allowed."; }
      else if (/[^a-z0-9_-]/.test(value)) { userid.setCustomValidity("Only letters, numbers, underscore, dash."); userErr.textContent = "Only letters, numbers, _, - allowed."; }
      else { userid.setCustomValidity("Invalid User ID format."); userErr.textContent = "Invalid format."; }
      return false;
    }
    userid.setCustomValidity(""); userErr.textContent = ""; return true;
  };
  ...
  const checkPasswords = () => {
    ...
    const p1 = pw.value; const p2 = pw2.value;
    ...
    else if (!/(?=.*[!@#%^&*()_+=\-\/><.,~`])/.test(p1)) msg = "Password must contain at least one special character (!@#%^&*()_+=-/><.,~`).";
    ...
    if (msg) { pw2.setCustomValidity(msg); pwErr.textContent = msg; }
    else { pw2.setCustomValidity(""); pwErr.textContent = ""; }
  };
  ...
  userid.addEventListener("input", () => { userErr.textContent = ""; checkPasswords(); });

  // ===== MI (preserve case; just validate) =====
  const formatMI = () => {
    mi.value = preventHTMLTags(mi.value); // no forced uppercase
    const v = mi.value.trim();
    if (v === "") { mi.setCustomValidity(""); miErr.textContent = ""; return; }
    if (!/^[A-Za-z]$/.test(v)) {
      mi.setCustomValidity("Middle Initial must be exactly one letter (A–Z) or blank.");
      miErr.textContent = "Must be one letter or blank.";
    } else { mi.setCustomValidity(""); miErr.textContent = ""; }
  };
  ...
  const validateLastName = () => {
    const value = preventHTMLTags(lname.value.trim()); lname.value = value;
    if (!value) { lname.setCustomValidity("Last Name is required."); lnameErr.textContent = "Last Name is required."; return false; }
    const pattern = /^[A-Za-z'2-5-]{1,30}$/;
    if (!pattern.test(value)) { lname.setCustomValidity("Last Name: Use letters, apostrophes, numbers 2-5, and dashes only."); lnameErr.textContent = "Only letters, apostrophes, numbers 2-5, and dashes allowed."; return false; }
    if (value.length > 30) { lname.setCustomValidity("Last Name must be 1-30 characters."); lnameErr.textContent = "Must be 1-30 characters."; return false; }
    lname.setCustomValidity(""); lnameErr.textContent = ""; return true;
  };
  ...
  function validateDOB() {
    const value = preventHTMLTags(dob.value.trim()); dob.value = value;
    const pattern = /^(0[1-9]|1[0-2])\/(0[1-9]|[12][0-9]|3[01])\/((19|20)\d{2})$/;
    if (!value) { dob.setCustomValidity("Date of Birth is required."); dobErr.textContent = "Date of Birth is required."; return false; }
    if (!pattern.test(value)) { dob.setCustomValidity("Please use MM/DD/YYYY format."); dobErr.textContent = "Please use MM/DD/YYYY format."; return false; }

    const [m,d,y] = value.split('/').map(Number);
    const daysInMonth = [31,28,31,30,31,30,31,31,30,31,30,31];
    if (m === 2) {
      const leap = (y % 4 === 0 && y % 100 !== 0) || (y % 400 === 0);
      if (d > (leap ? 29 : 28)) { dob.setCustomValidity("Invalid day for February."); dobErr.textContent = "Invalid day for February."; return false; }
    } else if (d > daysInMonth[m-1]) { dob.setCustomValidity("Invalid date."); dobErr.textContent = "Invalid date."; return false; }
    ...
    // STRICT: exactly 120 years ago OR older is NOT allowed
    ...
    dob.setCustomValidity(""); dobErr.textContent = ""; return true;
  }

  function validateFutureDateGeneric(el, errEl, label) {
    const value = preventHTMLTags(el.value.trim()); el.value = value;
    const pattern = /^(0[1-9]|1[0-2])\/(0[1-9]|[12][0-9]|3[01])\/((19|20)\d{2})$/;
    if (!value) { el.setCustomValidity(`${label} is required.`); if (errEl) errEl.textContent = `${label} is required.`; return false; }
    if (!pattern.test(value)) { el.setCustomValidity("Please use MM/DD/YYYY format."); if (errEl) errEl.textContent = "Please use MM/DD/YYYY format."; return false; }

    const [m,d,y] = value.split('/').map(Number);
    const daysInMonth = [31,28,31,30,31,30,31,31,30,31,30,31];
    if (m === 2) {
      const leap = (y % 4 === 0 && y % 100 !== 0) || (y % 400 === 0);
      if (d > (leap ? 29 : 28)) { el.setCustomValidity("Invalid day for February."); if (errEl) errEl.textContent = "Invalid day for February."; return false; }
    } else if (d > daysInMonth[m-1]) { el.setCustomValidity("Invalid date."); if (errEl) errEl.textContent = "Invalid date."; return false; }

    const { minFutureDate } = calculateDateRanges();
    const date = new Date(y, m-1, d);
    if (date <= minFutureDate) { el.setCustomValidity(`${label} must be after today.`); if (errEl) errEl.textContent = `${label} must be after today.`; return false; }

    if (!checkAgainstDataRange(el, errEl, label)) return false;

    el.setCustomValidity(""); if (errEl) errEl.textContent = ""; return true;
  }
  ...
  // Auto format MM/DD/YYYY as you type
  ...
  const validateMedicalId = () => {
    const value = preventHTMLTags(medicalId.value.trim()); medicalId.value = value;
    if (!value) { medicalId.setCustomValidity("Medical ID is required."); medicalIdErr.textContent = "Medical ID is required."; return false; }
    if (!/^[A-Za-z0-9]{5,20}$/.test(value)) { medicalId.setCustomValidity("Medical ID must be 5-20 alphanumeric characters."); medicalIdErr.textContent = "Must be 5-20 letters/numbers."; return false; }
    medicalId.setCustomValidity(""); medicalIdErr.textContent = ""; return true;
  };
  ...
  // ===== State =====
  const validateState = () => {
    if (!state.value) { state.setCustomValidity("Please select a state."); stateErr.textContent = "Please select a state."; return false; }
    state.setCustomValidity(""); stateErr.textContent = ""; return true;
  };
  ...
  function validateEmail() {
    const value = preventHTMLTags(email.value.trim()); email.value = value;
    const pat = /^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$/;
    if (!value) { email.setCustomValidity("Email is required."); emailErr.textContent = "Email is required."; return false; }
    if (!pat.test(value)) { email.setCustomValidity("Use format: name@domain.tld"); emailErr.textContent = "Use format: name@domain.tld"; return false; }
    email.setCustomValidity(""); emailErr.textContent = ""; return true;
  }
  email.addEventListener("input", validateEmail);
  email.addEventListener("blur", validateEmail);

  // ===== Phone =====
  const formatPhone = (d) => (d.length <= 3 ? d : d.length <= 6 ? `${d.slice(0,3)}-${d.slice(3)}` : `${d.slice(0,3)}-${d.slice(3,6)}-${d.slice(6,10)}`);
  function updatePhone(){
    const digits = preventHTMLTags(phone.value.replace(/\D/g, "")).slice(0,10);
    phone.value = formatPhone(digits);
    if (digits.length === 10) { phone.setCustomValidity(""); phoneErr.textContent = ""; }
    else { phone.setCustomValidity("Phone must be exactly 10 digits in format 000-000-0000."); }
  }
  function validatePhoneOnBlur(){
    const digits = phone.value.replace(/\D/g, "");
    if (digits.length !== 10) { phone.setCustomValidity("Phone must be exactly 10 digits in format 000-000-0000."); phoneErr.textContent = "Must be 10 digits: 000-000-0000"; }
    else { phone.setCustomValidity(""); phoneErr.textContent = ""; }
  }
  ...
  const validateAddress1 = () => {
    const value = preventHTMLTags(address1.value.trim()); address1.value = value;
    if (!value) { address1.setCustomValidity("Address Line 1 is required."); addr1Err.textContent = "Address Line 1 is required."; return false; }
    if (value.length < 2 || value.length > 30) { address1.setCustomValidity("Address Line 1 must be 2-30 characters."); addr1Err.textContent = "Must be 2-30 characters."; return false; }
    address1.setCustomValidity(""); addr1Err.textContent = ""; return true;
  };
  const validateAddress2 = () => {
    const value = preventHTMLTags(address2.value.trim()); address2.value = value;
    if (value && (value.length < 2 || value.length > 30)) { address2.setCustomValidity("If provided, Address Line 2 must be 2-30 characters."); addr2Err.textContent = "If entered, 2–30 chars."; return false; }
    address2.setCustomValidity(""); addr2Err.textContent = ""; return true;
  };
  const validateCity = () => {
    const value = preventHTMLTags(city.value.trim()); city.value = value;
    if (!value) { city.setCustomValidity("City is required."); cityErr.textContent = "City is required."; return false; }
    if (value.length < 2 || value.length > 30) { city.setCustomValidity("City must be 2-30 characters."); cityErr.textContent = "Must be 2-30 characters."; return false; }
    city.setCustomValidity(""); cityErr.textContent = ""; return true;
  };
  ...
  // ===== ZIP =====
  const validateZip = () => {
    const value = preventHTMLTags(zip.value.trim()); zip.value = value;
    if (!value) { zip.setCustomValidity("ZIP Code is required."); zipErr.textContent = "ZIP Code is required."; return false; }
    const pat = /^\d{5}(-\d{4})?$/;
    if (!pat.test(value)) { zip.setCustomValidity("Use 12345 or 12345-6789 format."); zipErr.textContent = "Use 12345 or 12345-6789 format."; return false; }
    // Truncate and rewrite field to first 5 digits (per requirement)
    const m = value.match(/\d{5}/);
    if (m) zip.value = m[0];
    zip.setCustomValidity(""); zipErr.textContent = ""; return true;
  };
  zip.addEventListener("input", validateZip);
  zip.addEventListener("blur", validateZip);

  // ===== Range & Money =====
  const updateRange = () => { rangeOut.textContent = `Current: ${range.value}`; };
  range.addEventListener("input", updateRange); updateRange();

  function updateSalary(){ salaryOut.textContent = `${money(Number(salary.value))} / year`; }
  function updatePrice(){
    let minV = Number(priceMin.value);
    let maxV = Number(priceMax.value);
    if (minV > maxV) { maxV = minV; priceMax.value = maxV; }
  ...
  function buildReview() {
    ...
    updatePhone(); validateZip(); validateEmail(); formatMI(); checkPasswords();
    updateSalary(); updatePrice(); validateDOB(); validateLastName(); validateState();
    const address1Valid = validateAddress1();
    const address2Valid = validateAddress2();
    const cityValid     = validateCity();
    const travelValid   = validateTravelDate();
    const moveInValid   = validateMoveInDate();
    const userOk        = validateUserID();
    const medOk         = validateMedicalId();

    const first = fname.value.trim();
    const mid   = mi.value.trim(); // preserve case
...
const symptomsVal = (symptoms?.value.trim() || "(none)")
    ...
    reviewBody.innerHTML = `
      ...
      <div class="rev-row">
        ...
        <span class="rev-status ${(fname.checkValidity() && validateLastName() && mi.checkValidity()) ? 'ok':'err'}">
          ${(fname.checkValidity() && validateLastName() && mi.checkValidity()) ? 'pass' : 'ERROR: Check name formats'}
        </span>
      </div>
      ...
      <div class="rev-row">
        ...
        <span class="rev-status ${ validateDOB() ? 'ok':'err'}">
          ${ validateDOB() ? 'pass' : 'ERROR: Invalid or out of range' }
        </span>
      </div>
      ...
      <div class="rev-row">
        ...
        <span class="rev-status ${ validateEmail() ? 'ok':'err'}">
          ${ validateEmail() ? 'pass' : 'ERROR: Invalid email' }
        </span>
      </div>
      ...
      <div class="rev-row">
        ...
        <span class="rev-status ${ phone.checkValidity() ? 'ok':'err'}">
          ${ phone.checkValidity() ? 'pass' : 'ERROR: Format 000-000-0000' }
        </span>
      </div>
...
${addrLines.join('\n')}
        ...
        <span class="rev-status ${
          (address1Valid && address2Valid && cityValid && validateState() && validateZip()) ? 'ok':'err'
        }">
          ${
            (address1Valid && address2Valid && cityValid && validateState() && validateZip())
            ? 'pass' : 'ERROR: Check address fields / ZIP'
          }
        </span>
      </div>
      ...
      <div class="rev-row">
        ...
        <span class="rev-status ${
          pw.checkValidity() && pw2.checkValidity() && pw.value===pw2.value &&
          !pw.value.toLowerCase().includes(userid.value.trim().toLowerCase()) &&
          !pw.value.toLowerCase().includes(fname.value.trim().toLowerCase()) &&
          !pw.value.toLowerCase().includes(lname.value.trim().toLowerCase())
        ? 'ok':'err'}">
          ${
            (pw.checkValidity() && pw2.checkValidity() && pw.value===pw2.value &&
             !pw.value.toLowerCase().includes(userid.value.trim().toLowerCase()) &&
             !pw.value.toLowerCase().includes(fname.value.trim().toLowerCase()) &&
             !pw.value.toLowerCase().includes(lname.value.trim().toLowerCase()))
            ? 'pass' : 'ERROR: Strength / match / contains name or id'
          }
        </span>
      </div>

      <div style="margin-top:14px; display:flex; gap:12px; justify-content:center; flex-wrap:wrap;">
        <button type="button" class="btn-review" id="btnReviewAgain">SUBMIT</button>
        <input type="reset" value="START OVER" />
      </div>
    `;

    document.getElementById("btnReviewAgain")?.addEventListener("click", () => form.requestSubmit());
  ...
  form.addEventListener("submit", (e) => {
    // Force lowercase conversion again at submit (requirement)
    ...
    if (ssn){
      ...
      if (ssnDigits.length === 9 && ssn.dataset.confirmed !== "true") {
        ...
        if (ssnConfirm){
          ...
          ssnConfirm.hidden = false; ssnYes.hidden = false; ssnNo.hidden = false; ssnEdit.hidden = true;
        }
        return;
      }
    }
